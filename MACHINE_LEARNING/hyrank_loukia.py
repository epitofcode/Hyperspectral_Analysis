"""
HyRANK Loukia - Spatial-Spectral Classification
==============================================================
...
Dataset: HyRANK Loukia - Urban/Suburban scene
Sensor: Hyperspectral imaging (249 x 945 x 176 bands)
...
print("="*80)
print("HYRANK LOUKIA - SIMPLIFIED BASELINE CLASSIFICATION")
print("="*80)
...
# Loukia has 14 classes but specific class names are not documented
# We'll use generic "Class N" labels
...
# ==================================================================
print("="*80)
print("STEP 1: LOADING HYRANK LOUKIA DATASET")
print("="*80)

data_dir = Path(__file__).parent.parent / 'data' / 'hyrank_loukia'
image = load_hyperspectral_mat(data_dir / 'loukia_image.mat')
gt = load_ground_truth(data_dir / 'loukia_gt.mat')
...
for class_id in unique_classes:
    ...
    print(f"  Class {class_id:2d}: {train_count:4d} train, {test_count:4d} test")
...
print("-" * 40)
for idx, class_id in enumerate(unique_classes):
    print(f"  Class {class_id:2d}: {per_class_acc[idx]:6.2f}%")
print("-" * 40)
...
output_dir = Path(__file__).parent / 'RESULTS' / 'hyrank_loukia'
output_dir.mkdir(parents=True, exist_ok=True)

# Save text results
results_file = output_dir / 'loukia_results.txt'
with open(results_file, 'w') as f:
    f.write("HYRANK LOUKIA - SIMPLIFIED BASELINE RESULTS\n")
    f.write("="*80 + "\n\n")
    ...
    for idx, class_id in enumerate(unique_classes):
        count = np.sum(gt == class_id)
        f.write(f"  Class {class_id:2d}: {count:5d} samples\n")
    ...
    f.write("-" * 40 + "\n")
    for idx, class_id in enumerate(unique_classes):
        f.write(f"  Class {class_id:2d}: {per_class_acc[idx]:6.2f}%\n")
...
axes[0, 1].set_title('Ground Truth\n(14 classes)', fontsize=12, fontweight='bold')
axes[0, 1].axis('off')
...
axes[2, 0].set_yticklabels([f"Class {cid}" for cid in unique_classes], fontsize=9)
...
plt.suptitle('HyRANK Loukia Classification - Simplified Baseline',
             fontsize=16, fontweight='bold', y=0.99)
plt.tight_layout(rect=[0, 0, 1, 0.98])

# Save figure
fig_file = output_dir / 'LOUKIA_RESULTS.png'
