"""
Indian Pines - Spatial-Spectral Classification
===============================================

Simplified baseline approach following CLAUDE.md pragmatism principle.

Dataset: Indian Pines - 16 agricultural classes
Sensor: AVIRIS (145 x 145 x 200 bands)
...
print("="*80)
print("INDIAN PINES - SIMPLIFIED BASELINE CLASSIFICATION")
print("="*80)
...
# Class names (16 agricultural classes)
class_names = [
...
data_dir = Path(__file__).parent.parent / 'data' / 'indian_pines'
image = load_hyperspectral_mat(data_dir / 'indian_pines_image.mat')
gt = load_ground_truth(data_dir / 'indian_pines_gt.mat')
...
for idx, class_id in enumerate(unique_classes):
    ...
    print(f"  Class {class_id:2d} ({class_names[idx]:30s}): {count:5d} samples")
...
for class_id in unique_classes:
    ...
    print(f"  Class {class_id:2d} ({class_names[class_id-1]:30s}): {train_count:4d} train, {test_count:4d} test")
...
print("-" * 70)
for idx, class_id in enumerate(unique_classes):
    print(f"  {class_id:2d}. {class_names[idx]:30s}: {per_class_acc[idx]:6.2f}%")
print("-" * 70)
...
output_dir = Path(__file__).parent / 'RESULTS' / 'indian_pines'
output_dir.mkdir(parents=True, exist_ok=True)

# Save text results
results_file = output_dir / 'indian_pines_results.txt'
with open(results_file, 'w') as f:
    f.write("INDIAN PINES - SIMPLIFIED BASELINE RESULTS\n")
    f.write("="*80 + "\n\n")
    ...
    for idx, class_id in enumerate(unique_classes):
        count = np.sum(gt == class_id)
        f.write(f"  Class {class_id:2d} ({class_names[idx]:30s}): {count:5d} samples\n")
    ...
    f.write("-" * 40 + "\n")
    for idx, class_id in enumerate(unique_classes):
        f.write(f"  {class_id:2d}. {class_names[idx]:30s}: {per_class_acc[idx]:6.2f}%\n")
...
rgb_image = select_rgb_bands(image)
...
axes[0, 1].set_title('Ground Truth\n(16 agricultural classes)', fontsize=12, fontweight='bold')
axes[0, 1].axis('off')
...
axes[2, 0].set_yticklabels([f"{cid}. {class_names[cid-1][:20]}" for cid in unique_classes], fontsize=8)
...
plt.suptitle('Indian Pines Classification - Simplified Baseline',
             fontsize=16, fontweight='bold', y=0.99)
plt.tight_layout(rect=[0, 0, 1, 0.98])

# Save figure
fig_file = output_dir / 'INDIAN_PINES_RESULTS.png'
