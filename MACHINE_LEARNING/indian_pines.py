"""
INDIAN PINES - Complete Hyperspectral Classification Pipeline
=============================================================
...
print("="*80)
print("INDIAN PINES - COMPLETE CLASSIFICATION PIPELINE")
print("="*80)
print("\nThis script will:")
...
fig.suptitle('INDIAN PINES - Complete Classification Results | ' +
...
ax6.set_yticklabels([f'{actual_classes[i]}. {name[:10]}' for i, name in enumerate(class_names)], fontsize=9)
...
ax7.axis('off')
legend_elements = [mpatches.Patch(facecolor=cmap(i), label=f'{actual_classes[i]}. {class_names[i][:15]}')
                   for i in range(n_actual_classes)]
...
dataset_text += f"Dataset: Indian Pines\n"
dataset_text += f"Location: Indiana, USA\n"
dataset_text += f"Scene: Agricultural\n\n"
...
dataset_text += f"Sensor: AVIRIS\n"
dataset_text += f"Wavelength: 400-2500nm\n"
dataset_text += f"Resolution: 20m"
ax9.text(0.5, 0.5, dataset_text, fontsize=10, verticalalignment='center',
         horizontalalignment='center', fontfamily='monospace',
         bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.9))

# Save visualization
output_dir = Path('../results/indian_pines')
output_dir.mkdir(parents=True, exist_ok=True)
output_path = output_dir / 'INDIAN_PINES_COMPLETE.png'
...
with open(results_file, 'w') as f:
    f.write("INDIAN PINES - COMPLETE CLASSIFICATION RESULTS\n")
    f.write("="*80 + "\n\n")
    ...
    for i, name in enumerate(class_names):
        f.write(f"  {actual_classes[i]:2d}. {name:30s}: {class_acc_ss[i]*100:5.2f}%\n")
