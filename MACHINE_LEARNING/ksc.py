"""
KSC (Kennedy Space Center) - Complete Hyperspectral Classification Pipeline
===========================================================================
...
print("="*80)
print("KSC (KENNEDY SPACE CENTER) - COMPLETE CLASSIFICATION PIPELINE")
print("="*80)
print("\nThis script will:")
...
print("\n[VISUALIZATION] Showing RGB image and Ground Truth...")
...
im = ax2.imshow(gt_masked, cmap='tab20')
...
plt.pause(0.1)
input("\n[OK] Press Enter to continue to Step 2 (PCA)...")
plt.close()
...
print("\n[VISUALIZATION] Showing PCA transformation...")
...
plt.show(block=False)
plt.pause(0.1)
input("\n[OK] Press Enter to continue to Step 3 (Pixel-wise Classification)...")
plt.close()
...
print("\n[VISUALIZATION] Showing pixel-wise classification results...")
fig, axes = plt.subplots(1, 3, figsize=(15, 4))
...
plt.show(block=False)
plt.pause(0.1)
input("\n[OK] Press Enter to continue to Step 4 (Spatial-Spectral with Patches)...")
plt.close()
...
print("\n[VISUALIZATION] Showing spatial-spectral classification results...")
fig, axes = plt.subplots(1, 3, figsize=(15, 4))
...
plt.show(block=False)
plt.pause(0.1)
input("\n[OK] Press Enter to see comparison...")
plt.close()
...
print("\n[VISUALIZATION] Showing comparison between methods...")
...
plt.show(block=False)
plt.pause(0.1)
input("\n[OK] Press Enter to continue to final visualization generation...")
plt.close()
...
print("\nClassifying entire image with spatial-spectral model...")
# For full image classification, we need to extract patches for all pixels
# For speed, we'll just use the test predictions as the classification map

classification_map = np.zeros((h, w), dtype=np.int32)
test_positions = labeled_positions[[i for i in range(len(labeled_positions)) if i % 10 == 7]][:len(y_test_ss)]
for pos, pred in zip(labeled_positions, y_all):
    classification_map[pos[0], pos[1]] = pred

# Set test positions to predictions
...
fig.suptitle('KSC (Kennedy Space Center) - Complete Classification Results | ' +
             f'Spatial-Spectral OA: {oa_ss*100:.2f}% | AA: {aa_ss*100:.2f}% | Kappa: {kappa_ss:.4f}',
             fontsize=18, fontweight='bold')

gs = fig.add_gridspec(3, 3, hspace=0.3, wspace=0.3)
cmap = plt.cm.get_cmap('tab20', n_actual_classes + 1)
...
ax5.set_xticklabels(actual_classes, fontsize=8)
ax5.set_yticklabels(actual_classes, fontsize=8)
for i in range(n_actual_classes):
    for j in range(n_actual_classes):
        ax5.text(j, i, f'{cm_display[i, j]:.1f}',
                ha="center", va="center",
                color="black" if cm_display[i, j] < 50 else "white", fontsize=7)
plt.colorbar(im5, ax=ax5, fraction=0.046)

ax6 = fig.add_subplot(gs[1, 2])
colors = ['green' if acc > 0.9 else 'orange' if acc > 0.7 else 'red' for acc in class_acc_ss]
bars = ax6.barh(range(n_actual_classes), class_acc_ss * 100, color=colors, alpha=0.7)
ax6.set_yticks(range(n_actual_classes))
ax6.set_yticklabels([f'{actual_classes[i]}. {name[:12]}' for i, name in enumerate(class_names)], fontsize=9)
...
for i, (acc, bar) in enumerate(zip(class_acc_ss, bars)):
    ax6.text(acc * 100 + 2, i, f'{acc*100:.1f}%', va='center', fontsize=8, fontweight='bold')
...
legend_elements = [mpatches.Patch(facecolor=cmap(i), label=f'{actual_classes[i]}. {class_names[i]}')
                   for i in range(n_actual_classes)]
ax7.legend(handles=legend_elements, loc='center', fontsize=9,
           title='Class Labels', title_fontsize=11, frameon=True, ncol=2)
...
ax8.text(0.5, 0.5, summary_text, fontsize=10, verticalalignment='center',
         horizontalalignment='center', fontfamily='monospace',
         bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.9))
...
dataset_text += f"Dataset: Kennedy Space Center\n"
dataset_text += f"Location: Florida, USA\n"
dataset_text += f"Scene: Wetlands\n\n"
...
dataset_text += f"Sensor: NASA AVIRIS\n"
dataset_text += f"Wavelength: 400-2500nm\n"
dataset_text += f"Resolution: 18m"
ax9.text(0.5, 0.5, dataset_text, fontsize=10, verticalalignment='center',
         horizontalalignment='center', fontfamily='monospace',
...
output_path = output_dir / 'KSC_COMPLETE.png'
...
with open(results_file, 'w') as f:
    f.write("KSC (KENNEDY SPACE CENTER) - COMPLETE CLASSIFICATION RESULTS\n")
    f.write("="*80 + "\n\n")
